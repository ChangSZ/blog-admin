        ...
        methods: {
            defaultData (id) {
                PostEdit(id)
                    .then(data => {
                        ...
                        this.contentEditor.setValue(data.data.data.post.post.Original);
                    })
                    .catch(() => {
                        console.log("err")
                    });
            ...
            handleSubmit (name) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        PostUpdate(that.postId,that.formValidate.title,that.formValidate.category,that.formValidate.tags,that.formValidate.summary,that.contentEditor.getValue())
                            .then(res => {
                                if (res.data.code === 0) {
