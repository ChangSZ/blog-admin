...
<script>
import { CateEdit, CateUpdate, getCateList } from "@/api/cate";
    ...
    return {
      ...
      ruleValidate: {
        ...
        displayName: [
          {
            required: true,
            message: "The displayName cannot be empty",
            trigger: "blur",
          },
          {
            max: 100,
            message: "The displayName length is too long",
            trigger: "blur",
          },
        ],
        parentCate: [
        ...
        seoDescription: [
          {
            required: true,
            message: "The seo description can not be empty",
            trigger: "blur",
          },
          {
            max: 250,
            message: "The seo description length is too long",
            trigger: "blur",
          },
        ],
      },
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          CateUpdate(
            ...
            that.formValidate.displayName,
            that.formValidate.seoDescription,
            that.formValidate.parentCate
          )
            .then((res) => {
              if (res.data.code === 0) {
